...
importMap = [
    ...
    [ "import {get_distance_sqr,get_distance,isInRect,isOverlapRect", "const { get_distance_sqr, get_distance, isInRect, isOverlapRect, tempPath } = Me.imports.utils;" ],
...
def dump(f):
    ...
    for l in open(f, "r"):
        ...
        m = re.match(r"^//\s*skip\s+next\s+(\d+)", l)
        if m:
            count = int(m.group(1))
            skip = count
            continue
